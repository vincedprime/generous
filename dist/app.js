...
function render(){
 ...
 document.getElementById('results').innerHTML=items.map(p=>`<article class="product"><div class="product-main"><div class="monogram" aria-hidden="true"><img src="logos/${p.name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}.png" alt="" width="28" height="28" loading="lazy" decoding="async"></div><div><div class="product-title"><a href="${escapeHtml(p.source)}" target="_blank" rel="noopener noreferrer">${escapeHtml(p.name)} <span class="external" aria-label="opens official source in new tab">↗</span></a></div><p class="product-category">${escapeHtml(p.category)} / ${escapeHtml(p.product)}</p><p class="description">${escapeHtml(p.summary)}</p></div><div class="score" aria-label="Draft editorial score ${p.score} out of 100"><span class="score-value">${p.score}</span><span class="score-max"> /100</span><div class="score-track" aria-hidden="true"><span style="width:${p.score}%"></span></div><div class="rank">#${categoryRank(p)} in category</div></div></div><div class="product-bottom"><span class="pill">${escapeHtml(p.tier)}</span>${kind(p)!=='hosted'?`<span class="pill">${kind(p)==='selfhost'?'Self-hosted':'Usage fees apply'}</span>`:''}<details><summary>Score breakdown</summary><div class="breakdown">${['Permanence','Capacity','Features','Commercial fit','No card'].map((label,i)=>`<span>${label}<strong>${p.dimensions[i]}/4</strong></span>`).join('')}</div></details></div></article>`).join('');
}
